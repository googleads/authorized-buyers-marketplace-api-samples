...
#
# Activates a client user with the given account, client, and user ID.
#
...
def activate_client_users(marketplace, options)
  name = "buyers/#{options[:account_id]}/clients/#{options[:client_id]}/users/#{options[:client_user_id]}"

  body = Google::Apis::AuthorizedbuyersmarketplaceV1::ActivateClientUserRequest.new()

  puts "Activating client user with name '#{name}':"

  client_user = marketplace.activate_client_user(name, body)
...
if __FILE__ == $0
  ...
  options = [
    Option.new(
      ...
      'construct the name used as a path parameter for the users.activate request.',
    ...
    Option.new(
      'client_user_id',
      'The resource ID of the buyers.clients.users resource that is being activated. This will be used to construct '\
      'the name used as a path parameter for the users.activate request.',
  ...
  begin
    activate_client_users(service, opts)
