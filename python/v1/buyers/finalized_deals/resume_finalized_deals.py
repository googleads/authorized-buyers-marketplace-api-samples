...
def main(marketplace, args):
    ...
    print(f'Resuming finalized deal with name "{finalized_deal_name}":')
    try:
        # Construct and execute the request.
        response = marketplace.buyers().finalizedDeals().resume(
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description=('Resume a finalized deal for the given buyer account ID '
                     'and deal ID.'))
    # Required fields.
    parser.add_argument(
        '-a', '--account_id', default=DEFAULT_BUYER_RESOURCE_ID,
        help=('The resource ID of the buyers resource for which the finalized '
              'deal is being resumed. This will be used to construct the name '
              'used as a path parameter for the finalizedDeals.resume request.'
              ))
    parser.add_argument(
        '-d', '--deal_id', default=DEFAULT_FINALIZED_DEAL_RESOURCE_ID,
        help=('The resource ID of the buyers.finalizedDeals resource that is '
              'being resumed. This will be used to construct the name used as '
              'a path parameter for the finalizedDeals.resume request.'))
