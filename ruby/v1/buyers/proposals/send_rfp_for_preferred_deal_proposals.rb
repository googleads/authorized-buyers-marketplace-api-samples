...
#
# Sends a request for proposal (RFP) to a publisher for a preferred deal.
#
...
def send_rfp_for_preferred_deal_proposals(marketplace, options)
  ...
  body = Google::Apis::AuthorizedbuyersmarketplaceV1::SendRfpRequest.new(
    ...
    note: 'Test preferred deal proposal created by Ruby sample.',
    ...
    preferred_deal_terms: Google::Apis::AuthorizedbuyersmarketplaceV1::PreferredDealTerms.new(
  ...
  puts "Creating preferred deal RFP on behalf of buyer account '#{parent}' to publisher profile with name "\
        "#{publisher_profile_name}"

  preferred_deal_proposal = marketplace.send_proposal_rfp(parent, body)
  ProposalPrinter.print(preferred_deal_proposal)
end
...
if __FILE__ == $0
  ...
  options = [
    ...
    Option.new(
      ...
      short_alias: 'd', required: false, default_value: "Test PD Proposal #{SecureRandom.uuid}"
    ),
  ]
  ...
  begin
    send_rfp_for_preferred_deal_proposals(service, opts)
