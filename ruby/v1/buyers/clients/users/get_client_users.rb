...
def get_client_users(marketplace, options)
  name = "buyers/#{options[:account_id]}/clients/#{options[:client_id]}/users/#{options[:client_user_id]}"

  puts "Get client user with name '#{name}'"

  client_user = marketplace.get_buyer_client_user(name)
...
if __FILE__ == $0
  ...
  options = [
    Option.new(
      ...
      'construct the name used as a path parameter for the users.get request.',
    ...
    Option.new(
      'client_user_id',
      'The resource ID of the buyers.clients.users resource that is being retrieved. This will be used to construct '\
      'the name used as a path parameter for the users.get request.',
