...
def list_finalized_deals(marketplace, options)
  ...
  puts "Listing finalized deals for bidder with name: \"#{name}\"."
  begin
    response = marketplace.list_bidder_finalized_deals(
        name,
...
if __FILE__ == $0
  ...
  options = [
    Option.new(
      'account_id',
      'The resource ID of the bidders resource for which the finalized deals are being retrieved. This will be used '\
